...
export function createMotionField(canvas, options = {}) {
  ...
  // ---- grace sigil (hero backdrop ring + cross, subtle rotating) ----
  let sigil = null;
  if (graceSigil) {
    sigil = new THREE.Group();
    const ringMat = new THREE.MeshBasicMaterial({ color: 0xe7b878, transparent: true, opacity: 0, toneMapped: false });
    const ring = new THREE.Mesh(new THREE.RingGeometry(0.985, 1, 96), ringMat);
    sigil.add(ring);
    const barMat = new THREE.MeshBasicMaterial({ color: 0xe7b878, transparent: true, opacity: 0, toneMapped: false });
